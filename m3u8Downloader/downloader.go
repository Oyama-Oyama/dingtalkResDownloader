...
var slicRequestConfigs map[string]string
...
func init() {
	...
	_ = json.Unmarshal(byteData, &slicRequestConfigs)
	fmt.Println("request configs:", slicRequestConfigs)

	byteData2, _ := os.ReadFile("cookies.json")
	_ = json.Unmarshal(byteData2, &cookies)
...
func (md *m3u8downloader) httpGetBodyToByte(url string) []byte {
	...
	// for key, value := range slicRequestConfigs {
...
func (md *m3u8downloader) MergeFileInDir(path string, saveName string) error {
	...
	fmt.Println("merege all ts file:", fileList)
...
func (md *m3u8downloader) MergeFile() error {
	...
	fmt.Println("merge file:", md.config.SaveDirectory, md.suffixList, md.config.VideoName)
